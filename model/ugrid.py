...
class UGrid(paddle.nn.Layer):
    def __init__(self,
        ...
        if 'none' in self.activation:
            ...
            self.activate = lambda x: paddle.clip(x, float(low), float(high))
    ...
    #
    #     r = util.absolute_residue(y, bc_mask, f, reduction='none').reshape(y.shape)
    #
...
class UNetSkipConnectionBlock(paddle.nn.Layer):
    ...
    def __init__(self,
                 ...
                 initialize_trainable_parameters: str,
                 submodule: typing.Optional[paddle.nn.Layer]):
        super().__init__()
        ...
        self.submodule: typing.Optional[paddle.nn.Layer] = submodule

        # Convolution (linear) layers
        self.pre_smoothers = paddle.nn.LayerList(
                [paddle.nn.Conv2D(1, 1, 3, stride=1, padding=1, bias_attr=False) for _ in range(self.num_pre_smoothing)])

        if self.downsampling_policy == 'lerp':
            self.downsampler = util.downsample2x
        elif self.downsampling_policy == 'conv':
            self.downsampler = paddle.nn.Conv2D(1, 1, 3, stride=2, padding=1, bias_attr=False)
        else:
            raise NotImplementedError
        ...
        if self.upsampling_policy == 'lerp':
            ...
            self.upsampler = paddle.nn.Conv2DTranspose(1, 1, 3, stride=2, padding=1, bias_attr=False)
        else:
            raise NotImplementedError

        self.post_smoothers = paddle.nn.LayerList(
                [paddle.nn.Conv2D(1, 1, 3, stride=1, padding=1, bias_attr=False) for _ in range(self.num_post_smoothing)])

        # Initialization of trainable parameters
        if self.initialize_trainable_parameters == 'default':
            # PaddlePaddle's default initialization.
            # For Conv2D, uses kaiming initialization.
            self.initialize: typing.Optional[typing.Callable[[paddle.Tensor], None]] = None
        else:
            raise NotImplementedError
    ...
    def forward(self, x: paddle.Tensor, interior_mask: paddle.Tensor) -> paddle.Tensor:
