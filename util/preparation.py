...
# noinspection DuplicatedCode
def square_region_test(laplacian: float, device: str) \
    ...
    bc_value: paddle.Tensor = paddle.to_tensor(bc_value).unsqueeze(0).unsqueeze(0)
    bc_mask: paddle.Tensor = paddle.to_tensor(bc_mask).unsqueeze(0).unsqueeze(0)
    ...
    if f is not None:
        f: paddle.Tensor = paddle.to_tensor(f).unsqueeze(0).astype('float32')
        if device == 'gpu':
            f = f.cuda()
